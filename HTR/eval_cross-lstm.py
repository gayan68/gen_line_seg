...
from src.model.models_utils import load_pretrained_model_LSTM
import json
...
parser.add_argument("--path_model_cnn", default="", type=str)
parser.add_argument("--path_model_lstm", default="", type=str)
...
def evaluate_test(args, config_values, device, char_list, char_dict, ctc_loss_fn, text_read):
    ...
    if os.path.isfile(args.path_model_cnn) and os.path.isfile(args.path_model_lstm):
        load_pretrained_model_LSTM(args.path_model_cnn, args.path_model_lstm, model_reco, device)
    else: 
        print("No model found at " + args.path_model_cnn)
        print("No model found at " + args.path_model_lstm)
...
if __name__ == '__main__':
    ...
    for key_train, value_train in trained_on.items():
        ...
        args.path_model_cnn = f"../../logs/CRNN_Center_loss/DomainAdaptation/{value_train['crnn_model']}/crnn_best.torch"
        args.path_model_lstm = f"../../logs/CRNN_Center_loss/DomainAdaptation/018_2026-04-07_ID_16124659/crnn_best.torch"
